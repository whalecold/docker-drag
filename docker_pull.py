...
print(
    "repo: {} tag: {} repository: {} registry: {} img: {}".format(
        repo, tag, repository, registry, img
    )
)
...
if resp.status_code == 401:
    ...
    try:
        ...
        reg_service = ""
print("auth_url {} reg service: {}".format(auth_url, reg_service))
# Get Docker token (this function is useless for unauthenticated registries like Microsoft)
def get_auth_head(type):
    url = "{}?service={}&scope=repository:{}:pull".format(
        auth_url, reg_service, repository
    )
    print("get auth head {}".format(url))
    resp = requests.get(url, verify=False,)
    print("get auth head {}".format(resp.status_code))
...
print("registry : {} repository: {} tag: {}".format(registry, repository, tag))
# Fetch manifest v2 and get image layer digests
auth_head = get_auth_head("application/vnd.docker.distribution.manifest.v2+json")
url = "https://{}/v2/{}/manifests/{}".format(registry, repository, tag)
...
print("resp string {}".format(resp.json()))
...
for layer in layers:
    ...
    print("fake layer id {}".format(fake_layerid))
    # Creating json file
